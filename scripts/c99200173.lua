...
function c99200173.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local b1=e:GetHandler():IsCanRemoveCounter(tp,0x106,3,REASON_COST)
		and Duel.IsExistingMatchingCard(c99200173.filter1,tp,LOCATION_MZONE,0,1,nil)
	local b2=e:GetHandler():IsCanRemoveCounter(tp,0x106,5,REASON_COST)
	...
	if op==0 then
		...
		e:GetHandler():RemoveCounter(tp,0x106,3,REASON_COST)
	else
		e:SetCategory(CATEGORY_TOHAND)
		e:GetHandler():RemoveCounter(tp,0x106,5,REASON_COST)
